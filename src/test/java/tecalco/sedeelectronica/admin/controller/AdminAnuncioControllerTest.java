...
public class AdminAnuncioControllerTest {
	...
	public void getEdit() throws Exception {
		try {
			this.mvc.perform(post("/admin/anuncios/edit/2")).andExpect(status().isOk());
		} catch (Exception e) {
	...
	public void getDelete() throws Exception {
		try {
			this.mvc.perform(post("/admin/anuncios/delete/3")).andExpect(status().isInternalServerError());
		} catch (Exception e) {
			assertThat(e.getMessage(), is(
					"Request processing failed; nested exception is org.springframework.dao.EmptyResultDataAccessException: No class teralco.sedeelectronica.model.Anuncio entity with id 3 exists!"));
		}
	}

	@Test
	public void getSave() throws Exception {
		this.mvc.perform(get("/admin/anuncios/save")).andExpect(status().isMethodNotAllowed());
	}
