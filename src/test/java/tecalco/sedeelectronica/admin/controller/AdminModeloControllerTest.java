...
public class AdminModeloControllerTest {
	...
	public void getEdit() throws Exception {
		try {
			this.mvc.perform(post("/admin/modelos/edit/2")).andExpect(status().isOk());
		} catch (Exception e) {
	...
	public void getDelete() throws Exception {
		try {
			this.mvc.perform(post("/admin/modelos/delete/3")).andExpect(status().isInternalServerError());
		} catch (Exception e) {
	...
	public void getSave() throws Exception {
		this.mvc.perform(get("/admin/modelos/save")).andExpect(status().isMethodNotAllowed());
	}
