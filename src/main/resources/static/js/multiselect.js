...
$(document).ready(function() {    
    ...
    $.getJSON("/sede/js/idiomas.json", function(result) {
        var options = $('#search');
        //don't forget error handling!
        var optionsFill=[];
        $('#search_to option').each(function(i) {
            optionsFill.push($(this).val());
        });
        $.each(result, function(i, item) {
            if (!optionsFill.includes(this.codigo))
            {
                options.append($("<option />").val(this.codigo).text(this.idioma+" - "+this.codigo));
            }
        });
    });


    $("#lenguajes-form").submit(function (event) {
...
function lenguajes_submit() {
    var lenguajes = [];
    
    $("#search_to option").each(function(i) {
        lenguajes.push($(this).text());
    });
    $("#feedback").stop(true, true).fadeIn().stop(true, true);
    $.ajax({
        type: "POST",
        contentType: "application/json",
        url: "/sede/admin/lenguajes/create",
        data: JSON.stringify(lenguajes),
        dataType: 'json',
        cache: false,
        timeout: 600000,
        success: function (data) {
            var json = "<div class='alert alert-success'>" +
                    "<strong>Lenguajes</strong> guardados correctamente.</div>";
            $('#feedback').html(json);
            $("#feedback").fadeOut(1600 );    


            console.log("SUCCESS : ", data);
        },
        error: function (e) {
            $('#feedback').html("<div class='alert alert-warning'><strong>Error</strong> "+ 
                e.responseJSON.msg +"</div>");
        ...
        **/
        ...
        Multiselect.prototype = {
            ...
            moveToLeft: function( $options, event, silent, skipStack ) {
                ...
                {
                	$("#feedback").stop(true, true).fadeIn().stop(true, true);
                    $('#feedback').html("<div class='alert alert-warning'><strong>Error</strong> Debe haber un idioma seleccionado.</div>");
                    $("#feedback").fadeOut(4000 );
                }
            ...
            moveFromAtoB: function( $source, $destination, $options, event, silent, skipStack ) {
                ...
                $("#alertMsg").addClass("warning-white");
                $("#alertMsg").html("&nbsp;");
